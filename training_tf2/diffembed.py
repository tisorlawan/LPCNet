...
class diff_Embed(Layer):
    ...
    def __init__(
        self, units=128, dict_size=256, pcm_init=True, initializer=None, **kwargs
    ):
    ...
    def build(self, input_shape):
        ...
        self.w = tf.Variable(
            initial_value=w_init(shape=(self.dict_size, self.units), dtype="float32"),
            trainable=True,
        )
    ...
    def call(self, inputs):
        ...
        M = (1 - alpha) * tf.gather(self.w, inputs) + alpha * tf.gather(
            self.w, tf.clip_by_value(inputs + 1, 0, 255)
        )
        return M
