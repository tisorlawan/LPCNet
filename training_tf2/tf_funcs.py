...
def tf_u2l(u):
    ...
    s = K.sign(u)
    u = K.abs(u)
    return s * scale_1 * (K.exp(u / 128.0 * K.log(256.0)) - 1)
    ...
    def call(self, inputs, lpcoeffs_N=16, frame_size=160):
        ...
        cX = Lambda(
            lambda x: K.concatenate(
                [
                    x[:, (lpcoeffs_N - i) : (lpcoeffs_N - i + 2400), :]
                    for i in range(lpcoeffs_N)
                ],
                axis=2,
            )
        )
...
class diff_rc2lpc(Layer):
    def call(self, inputs, lpcoeffs_N=16):
        def pred_lpc_recursive(input):
            temp = input[0] + K.repeat_elements(
                input[1], input[0].shape[2], 2
            ) * K.reverse(input[0], axes=2)
        ...
        for i in range(1, lpcoeffs_N):
            lpc_init = Llpc(
                [lpc_init[:, :, :i], K.expand_dims(inputs[:, :, i], axis=-1)]
            )
        return lpc_init
...
class diff_lpc2rc(Layer):
    def call(self, inputs, lpcoeffs_N=16):
        def pred_rc_recursive(input):
            ki = K.repeat_elements(
                K.expand_dims(input[1][:, :, 0], axis=-1), input[0].shape[2], 2
            )
