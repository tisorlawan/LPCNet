...
def my_loss1(y_true, y_pred):
    return 1 * K.mean(K.square(y_true - y_pred)) + 1 * K.max(
        K.square(y_true - y_pred), axis=1
    )


def my_loss2(y_true, y_pred):
    return 0.1 * K.mean(K.square(y_true - y_pred)) + 1 * K.max(
        K.square(y_true - y_pred), axis=1
    )


def my_loss3(y_true, y_pred):
    return 0.01 * K.mean(K.square(y_true - y_pred)) + 1 * K.max(
        K.square(y_true - y_pred), axis=1
    )
...
def ratio(x):
    return tf.minimum(1.0, tf.maximum(-1.0, x[0] * x[1] / x[2]))
