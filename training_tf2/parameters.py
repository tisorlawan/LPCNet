...
def set_parameter(model, parameter_name, parameter_value, dtype="float32"):
    ...
    weights = [
        weight for weight in model.weights if weight.name == (parameter_name + ":0")
    ]
    ...
    if len(weights) == 0:
        model.add_weight(
            parameter_name,
            trainable=False,
            initializer=tf.keras.initializers.Constant(parameter_value),
            dtype=dtype,
        )
    elif len(weights) == 1:
        ...
        raise ValueError(
            f"more than one weight starting with {parameter_name}:0 in model"
        )
